...
Ham = eps1 * np.kron(np.kron(sigz, np.eye(2)), np.eye(m[0])) \
    + eps2 * np.kron(np.kron(np.eye(2), sigz), np.eye(m[0])) \
    + g * np.kron(np.kron(sigp, sigm), np.eye(m[0])) \
    + g * np.kron(np.kron(sigm, sigp), np.eye(m[0])) \
    + om1 * np.kron(np.kron(np.eye(2), np.eye(2)), ata(m[0])) \
    + la1 * np.kron(np.kron(sigx, np.eye(2)), atpa(m[0])) \
    + la1 * np.kron(np.kron(np.eye(2), sigx), atpa(m[0]))
    
# Interaction Hamiltonian between RCs and residual baths, system OPs
Vr = np.kron(np.kron(np.eye(2), np.eye(2)), atpa(m[0]))
time2 = time.time()
print("# Time Hamiltonian:", time2 - time1)

# Observable
Sz = np.kron(sigz, np.eye(2))
Sz = Sz + np.kron(np.eye(2), sigz)
...
rho = partial_trace(SStateR, [0], [4, rc])
...
print(rho)
print()
red_rho = partial_trace(rho, [0], [2,2])
print(red_rho)
