...
parser.add_argument('--delta', action='store', dest='delta', type=float, help='Coupling')

args = parser.parse_args()

la_h = 1.0 # coupling spin and hot bath
la_c = 1.0 # coupling spin and cold bath
eps_h = 1.0 # Hot qubit self-energy
eps_c = eps_h + args.delta # Cold qubit self-energy
g = 0.00025 # Inter-qubit coupling
...
Ham = np.kron(eps_h * sigp.dot(sigm), iden) + np.kron(iden, eps_c * sigp.dot(sigm))\
      + g * np.kron(sigp, sigm) + g * np.kron(sigm, sigp)

# Interaction Hamiltonian between RCs and residual baths, system OPs
Vh = la_h * np.kron(sigx, iden)
Vc = la_c * np.kron(iden, sigx)
time2 = time.time()
print("# Time Hamiltonian:", time2 - time1)

def brownian_spectrum_h(w):
    t_h = 2.0 # temperature of left bath
...
def brownian_spectrum_c(w):
    t_c = 0.1 # temperature of right bath
    #ga = 0.0071 / np.pi # residual coupling
    ga = 0.001 / np.pi
...
print(np.reshape(SState,(4,4)))

Dt = np.reshape(RhTensor.dot(SState),(4,4))
Hd = np.diag(H.eigenenergies())
print("%.6f %.14f" % (args.delta, np.real(np.trace(Hd.dot(Dt)))))
