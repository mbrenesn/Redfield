...
// Strong Coupling
// Rectification
// Asymmetry is in the coupling to the baths
// Only the cold bath is strongly coupled
/*******************************************************************************/
...
int main(int argc, char **argv)
{
  ...
  double om_h = 25.0; double om_c = 25.0; // Self-energy of the RCs

  // Parsing arguments
  if(argc != 13){
    std::cerr << "Usage: " << argv[0] << " --rc [rc] --jx [jx] --jy [jy] --jz [jz] --la [la] --dt [dt]" << std::endl;
    exit(1);
  }
  ...
  if(jx == 0.777 || jy == 0.777 || jz == 0.777 || la == 0.777 || dt == 0.777 || rc == 777){
    std::cerr << "Error setting parameters" << std::endl;
    std::cerr << "Usage: " << argv[0] << " --rc [rc] --jx [jx] --jy [jy] --jz [jz] --la [la] --dt [dt]" << std::endl;
    exit(1);
  }

  double eps_h = 1.0; double eps_c = 1.0; // Local term H two-qubit
  double la_h = 1.0; double la_c = la; // System-Reservoir couplings
  double t_0 = 2.0;
  ...
  std::cout << "# Only one bath is strongly coupled" << std::endl;
  ...
  redT.construct_redfield_2r_phonon(Coh, RedL, RedR, Ham, Vl, Vr, 
          &brownian_spectrum, &ohmic_spectrum, t_h, t_c);
  redTr.construct_redfield_2r_phonon(Cohr, RedLr, RedRr, Hamr, Vlr, Vrr, 
          &brownian_spectrum, &ohmic_spectrum, t_c, t_h);
